...
def get_vehicle_filepath(idx: int) -> str:
...
def get_ego_vehicle_filepath() -> str:
...
def main() -> int:
    ...
    for i in range(NUM_VEHICLES):
        with open(get_vehicle_filepath(i), "w") as file_object:
            file_object.write("Lane: " + str(vehicle_datas[i]["Lane"]) + "\n")
            file_object.write("Distance: " + str(vehicle_datas[i]["Distance"]) + "\n")
    ...
    with open(get_ego_vehicle_filepath(), "w") as file_object:
